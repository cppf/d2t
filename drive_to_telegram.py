...
async def _upload_one(
    ...
    try:
        ...
        # Telethon calls this callback repeatedly while uploading.  Throttle
        # terminal output so concurrent uploads remain readable.
        progress_state = {"last": -1}
        def upload_progress(sent, total_bytes):
            if not total_bytes:
                return
            percent = int(sent * 100 / total_bytes)
            if percent != progress_state["last"] and (percent % 2 == 0 or percent == 100):
                progress_state["last"] = percent
                print(f"  [{index}/{total}] {percent:3d}% uploaded ({sent / (1024*1024):.1f}/{total_bytes / (1024*1024):.1f} MB)", flush=True)
